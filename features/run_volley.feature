...
Feature: Run Volley
  ...
  Scenario: Run a volley with stored response body
    ...
    Given I have a Volley with: '{ "name": "joe", "strikes": 1, "delay": 0 }'
    And I have an unauthenticated Target for that Attacker with priority of 100 and url of '{target_host}/public/1'
    When I send a PATCH request to "{se_host}/api/v1/sieges/{siege_id}/volleys/{volley_id}/start"
    Then the response status should be "200"
    And if I wait for volley to complete
    And the status of the Volley should be 'done' and there should be 1 results
    When I send a GET request to "{se_host}/api/v1/volleys/{volley_id}/results"
    Then the response status should be "200"
    And the JSON response should have 1 elements matching "$.results[?(@.code == 200)]"
    And the JSON response should have 1 elements matching "$.results[?(@.body == 'hello world')]"
